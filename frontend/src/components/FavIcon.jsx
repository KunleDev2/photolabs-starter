...
const FavIcon = ({displayAlert, selected, isModalClicked, dataCount}) => {
const [isLikedPhoto, setLikePhoto] = useState(false);
const [isLikedModal, setLikeModal] = useState(false);
const [alertCount, setAlertCount] = useState(0);
// const iconCount = selected.length;

  const doLikedPhoto = () => {
    // isLikedPhoto ? setLikePhoto(false) : setLikePhoto(true);
    
      // setAlertCount(dataCount.length);

    setLikePhoto(prevLiked => !prevLiked);
    setLikeModal(true);
  ...
  if (isModalClicked === true) {
    console.log("is modal clicked",isModalClicked);
    selected = true;
  }
